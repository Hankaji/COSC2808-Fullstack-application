...
export const getUsers = async (req: Request, res: Response) => {
  try {
    ...
    // Return the users with their relationship status
...
export const getUserById = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findById(id)
      .select("_id username displayName email profileImage friends")
      .lean();
    ...
    const friends = user.friends.map((friend: any) => ({
      ...
      virtualProfileImage:
        ...
        friend.profileImage.data
          ? `data:${
              friend.profileImage.contentType
            };base64,${friend.profileImage.data.toString("base64")}`
          : undefined,
    }));
